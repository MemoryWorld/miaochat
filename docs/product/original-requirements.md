...
> 来源说明
>
> - 迁移来源：`/home/torch/AgentHub_multiagentcowork/requirements.md`
> - 整理时间：2026-05-21
> - 用途：保存原始课题说明的可检索 Markdown 版本，作为产品需求背景材料

## 课题信息

- 课题名称：AgentHub - 多 Agent 协作平台
...
平台同时接入市面主流 Agent 平台，如 Claude Code、Codex、OpenCode 等，通过统一的适配器层屏蔽 API 差异，并支持用户自建 Agent。所有 Agent 产出，包括代码、网页、文档、PPT 等，支持实时预览、代码二次编辑和一键部署发布。
...
- 统一适配器层，至少接入 2 个主流 Agent 平台，如 Claude Code + Codex / OpenCode
- 支持用户自建 Agent，对话式创建并设定 System Prompt 与工具集
- 每个 Agent 在聊天列表中显示为独立的“联系人”，有头像、名称、能力标签

### 4. 产物预览与编辑

- Agent 回复中内联产物预览卡片，如网页 iframe、文档渲染、`[P2]` PPT 浏览
- 点击卡片展开全屏预览或代码编辑器
- `[P2]` 支持 Diff 视图、版本历史、对话式局部修改，例如选中代码后在聊天中描述修改
