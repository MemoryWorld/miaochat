...
describe("useActiveWorkspace", () => {
  ...
  it("retries the initial workspace load when auth-shaped failure is contradicted by an authenticated session", async () => {
    let workspaceFetchCount = 0;
    ...
    fetchMock.mockImplementation(async (input) => {
      ...
      if (url.endsWith("/workspaces")) {
        ...
        if (workspaceFetchCount === 1) {
          return new Response(JSON.stringify({ message: "请先登录后再继续操作。" }), {
            headers: {
              "Content-Type": "application/json"
            },
            status: 401
  ...
  it("keeps the user out of the login state when the initial retry still fails but the session is authenticated", async () => {
    let workspaceFetchCount = 0;
