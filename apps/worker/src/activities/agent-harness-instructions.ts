...
function formatMultiAgentChannelContract(): string {
  return [
    ...
    `- 如果用户要求 PPT、演示文稿、PowerPoint 或 presentation，必须在 envelope 的 tool_plan intent 中提出低风险工具调用 ${runtimePptxArtifactToolName}；系统会渲染为可下载的 .pptx 文件并自动附带在线放映版本。不要改用网页/幻灯片工具，也不要在正文里贴大段 HTML。`,
    '- PPT 工具唯一允许的 input 形状是 {"title":"标题","fileName":"可选文件名.pptx","slides":[{"title":"页标题","subtitle":"可选副标题","bullets":["要点 1","要点 2"],"notes":"可选演讲者备注"}]}。第一页是封面页（建议只给 title/subtitle）；正文页用 bullets 列要点，每页不超过 12 条；不要把整段长文塞进单条 bullet。',
    `- 仅当用户明确要求「网页幻灯片 / HTML 幻灯片」时才使用 ${runtimeSlidesArtifactToolName}。`,
    '- 幻灯片工具唯一允许的 input 形状是 {"title":"标题","fileName":"可选文件名","html":"完整单文件 HTML 幻灯片"}。',
    "- 幻灯片 HTML 要求：单文件内联全部 CSS/JS；每页是一个 16:9 的 <section class=\"slide\">；内置 ←/→/空格 键盘翻页与点击翻页；右下角显示「当前页/总页数」；首页为封面页；不依赖外部网络资源。",
    ...
    "  ]",
    "}",
    "- 产物工具调用的 envelope 必须使用 tool_plan intent，形如（字段名必须完全一致）：",
    "{",
    '  "visibleMessage": "给用户看的简短说明",',
    '  "intents": [',
    "    {",
    ...
    '      "summary": "创建请求的产物",',
    '      "calls": [',
    '        { "toolName": "artifact.pptx.create", "input": { "title": "标题", "slides": [ { "title": "封面标题", "subtitle": "副标题" }, { "title": "页标题", "bullets": ["要点 1", "要点 2"], "notes": "演讲者备注" } ] } }',
    "      ]",
    "    }",
    "  ]",
    "}",
